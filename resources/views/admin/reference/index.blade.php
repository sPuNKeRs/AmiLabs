            ...
            @foreach($researchs as $type)
                <tr id="type_{{$type->id}}" data-typeid="{{ $type->id }}">
                    ...
                    <td>
                        ...
                        <a href="#" data-typeid="{{ $type->id }}" data-toggle="tooltip" data-placement="bottom" data-original-title="Список анализов" class="btn action-btn analyzes-list bg-blue waves-effect">
  ...
  <script type="text/javascript">
    $(document).ready(function(){
      ...
      $('#type_table').on('click', '.edit-type', function(e){
        ...
        getResearchById(selectedType);
      });

      // Клик по кнопке список анализов
       $('#type_table').on('click', '.analyzes-list', function(e){
        var href = '{{ route('reference.research.analyzes') }}/' + $(e.currentTarget).data('typeid');
      ...
      $('#btn-save').on('click', function(e){
        ...
        $.ajax({
          url: '{{ route('reference.research.add') }}',
...
// ============================= ФУНКЦИИ ===================================
  ...
  function showConfirmMessage(type_id) {
    swal({
        ...
        deleteResearchById(type_id);
    });
  }

  // Удалить вид исследования по ID
  function deleteResearchById(type_id)
  {
    // Инициализация переменных
    var token = $('meta[name="_token"]').attr('content');
    ...
    $.ajax({
      url: '{{ route('reference.research.delete') }}',
      type: 'POST',
      headers: {'X-CSRF-TOKEN': token},
  ...
  function getResearchById(type_id)
  {
    // Инициализация переменных
    ...
    $.ajax({
      url: '{{ route('reference.research.edit') }}/' + type_id,
      type: 'GET',
      success: function (response) {
  ...
  function updateResearchById(type_id)
  {
    // Инициализация переменных
    var token = $('meta[name="_token"]').attr('content');
    ...
    $.ajax({
      url: '{{ route('reference.research.update') }}',
      headers: {'X-CSRF-TOKEN': token},
      processData: false,
      contentType: false,
      data: formData,
